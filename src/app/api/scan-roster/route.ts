...
import { readImage, readImageRequest, visionErrorResponse } from '@/lib/vision';

/**
 * Reads a screenshot of one day's roster — a person per row with the hours they
 * are on — and returns it as entries the Shifts page can preview before
 * anything is written.
 *
 * The screenshot rarely carries a date, so the date is chosen in the app; only
 * a date printed on the screenshot itself is reported here.
 */
...
const ROSTER_SCHEMA = {
  ...
  properties: {
    ...
    date: { ...TEXT, description: 'Only if a date is printed on the screenshot, as YYYY-MM-DD.' },
    rows: {
      type: 'array',
...
const INSTRUCTIONS = `This is a screenshot from a staff rostering app, showing one day's roster for one department or area.
...
Also return:
- dept: the heading naming the department or area, such as the title at the top of the screen.
- date: only if an actual date is printed somewhere on the screenshot. Leave it out otherwise — do not infer one.
...
export async function POST(req: NextRequest) {
  const request = readImageRequest(await req.json().catch(() => null));
  if (request instanceof NextResponse) return request;

  try {
    const { data, seconds, model } = await readImage<{ dept?: string; date?: string; rows?: ScannedRosterRow[] }>(
      request,
      INSTRUCTIONS,
      ROSTER_SCHEMA
    );
    ...
    if (!entries.length) {
      return NextResponse.json(
        { error: 'No rostered people could be read from that screenshot. Try again with the list in frame.' },
        { status: 422 }
      );
    }
    ...
    return visionErrorResponse(err, 'screenshot');
  }
}
