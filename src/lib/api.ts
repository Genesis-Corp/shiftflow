/**
 * Client-side fetch helpers for the Capture/roster-import flows.
 *
 * These are separate from apiClient.ts's fetchJson() deliberately: the ported
 * Capture UI (Staff and Shifts pages) was built against this exact
 * {ok, data, error} / {data, error} shape and postJson's abort-on-timeout
 * behaviour, which the vision routes need (a 47s server-side budget needs a
 * client that gives up before the platform kills the request outright).
 * Merging the two risked subtly changing behaviour in code that already
 * works; two small, differently-shaped helpers coexisting is the safer trade.
 */
